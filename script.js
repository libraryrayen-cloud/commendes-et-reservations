...
try {
  ...
  fbDb.ref('librairie/bookImages').on('value', snap => {
    const imgs = snap.val();
    if(!imgs) return;
    Object.assign(_imgCache, imgs);
    Object.keys(imgs).forEach(id=>idbSave(id,imgs[id]));
    Object.keys(booksDB).forEach(key=>{booksDB[key].forEach(b=>{if(_imgCache[b.id])b.img=_imgCache[b.id];});});
