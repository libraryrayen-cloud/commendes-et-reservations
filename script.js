...
try {
  ...
  fbDb.ref('librairie/fourHistory').on('value', snap => {
    fourHistory = snap.val() ? Object.values(snap.val()) : [];
    try{const d=JSON.parse(localStorage.getItem('librairie_rayen_db')||'{}');d.fourHistory=fourHistory;localStorage.setItem('librairie_rayen_db',JSON.stringify(d));}catch(e){}
    const adm = document.getElementById('adm-page');
    if(adm && adm.classList.contains('active') && curAT===6) renderFourHistory();
  });
...
let cart={};let curPay='visa';let orders=[],reservations=[];let fourHistory=[];let filtSchool='',filtLv='';let lastOrder=null;let _books=[];let orderEnabled=true;let curResPay='cash';
...
function renderFournisseur(){
  ...
  const rows=[];
  if(inclOrders)orders.forEach((r,i)=>rows.push({r,_type:'Commande',_idx:i}));
  if(inclResvs)reservations.forEach((r,i)=>rows.push({r,_type:'Réservation',_idx:i}));
  rows.sort((a,b)=>(b.r.id||0)-(a.r.id||0));
...
function renderFourHistory(){
  ...
  if(!tb)return;
  const rows=fourHistory.slice().reverse();
  if(!rows.length){tb.innerHTML=`<tr><td colspan="5" style="text-align:center;color:var(--tx3);padding:14px">Aucune commande fournisseur générée pour le moment.</td></tr>`;return;}
  tb.innerHTML=rows.map(h=>`<tr>
    <td style="font-size:.78rem">${h.date}</td>
    <td style="text-align:center">${h.titleCount}</td>
    <td style="text-align:center;font-weight:700;color:var(--orange)">${h.totalQty}</td>
    <td style="text-align:center">${h.sourceCount}</td>
    <td style="display:flex;gap:4px"><button class="del-row-btn" style="background:var(--navy);border-color:var(--navy)" onclick="viewFourHistory(${h.id})" title="Revoir / télécharger">👁️</button><button class="del-row-btn" onclick="deleteFourHistory(${h.id})" title="Supprimer">🗑️</button></td>
  </tr>`).join('');
}
function viewFourHistory(id){
  const h=fourHistory.find(x=>x.id===id);
  if(!h)return;
  _fourAgg=h.items;
  const tbody=document.getElementById('fourResBody');
  if(!tbody)return;
  ...
  const sumEl=document.getElementById('fourSummary');
  if(sumEl)sumEl.textContent=h.titleCount+' titre(s) différent(s) · '+h.totalQty+' exemplaire(s) au total · depuis '+h.sourceCount+' commande(s)/réservation(s) · généré le '+h.date;
  const res=document.getElementById('fourResult');
  if(res){res.style.display='block';res.scrollIntoView({behavior:'smooth',block:'start'});}
}
function deleteFourHistory(id){
  if(!confirm('Supprimer cette entrée de l\'historique ?'))return;
  fourHistory=fourHistory.filter(h=>h.id!==id);
  saveDataToStorage();
  renderFourHistory();
...
function searchBookLocations(query){
  const q=query.trim().toLowerCase();
  const resEl=document.getElementById('bookLocResults');
  if(!resEl)return;
  if(!q){resEl.innerHTML='';return;}
  const groups={};
  Object.keys(booksDB).forEach(key=>{
    const sep=key.indexOf('|');
    const school=key.slice(0,sep),level=key.slice(sep+1);
    booksDB[key].forEach(b=>{
      const hay=(b.title+' '+(b.ean||'')).toLowerCase();
      if(!hay.includes(q))return;
      const gkey=b.ean||('t:'+b.title.toLowerCase());
      if(!groups[gkey])groups[gkey]={title:b.title,ean:b.ean||'—',locations:[]};
      groups[gkey].locations.push({school,level});
    });
  });
  const list=Object.values(groups);
  if(!list.length){resEl.innerHTML=`<div style="padding:12px;color:var(--tx3);font-size:.82rem">Aucun livre trouvé.</div>`;return;}
  resEl.innerHTML=list.map(g=>`<div style="background:var(--slate);border-radius:var(--rs);padding:10px 14px;margin-bottom:8px"><div style="font-weight:700;color:var(--navy);font-size:.86rem">${g.title}</div><div style="font-size:.72rem;color:var(--tx3);margin-bottom:6px">EAN : ${g.ean} · ${g.locations.length} école(s)/niveau(x)</div><div style="display:flex;flex-wrap:wrap;gap:6px">${g.locations.map(l=>`<span style="background:white;border:1px solid var(--bd);border-radius:6px;padding:3px 9px;font-size:.75rem;cursor:pointer" data-school="${escAttr(l.school)}" data-level="${escAttr(l.level)}" onclick="jumpToBookEditor(this)" title="Ouvrir dans l'éditeur">🏫 ${l.school} — ${l.level}</span>`).join('')}</div></div>`).join('');
}
function jumpToBookEditor(el){
  const school=el.dataset.school,level=el.dataset.level;
  const sel=document.getElementById('edSch');if(sel){sel.value=school;updEdLv();}
  const lsel=document.getElementById('edLv');if(lsel){lsel.value=level;renderBookEd();}
  document.getElementById('bookEdArea')?.scrollIntoView({behavior:'smooth',block:'start'});
}
...
function saveDataToStorage(){
  ...
  const data={orders,reservations,fourHistory,schoolLevels,booksDB:booksDBNoImg,libName,libTag,libTel,libMF,libAddr,deliveryFee,remisePct,tvaPct,autoSave,adminUser,adminPass,heroSubTxt,deliveryNote};
